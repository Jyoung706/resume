...
// SQLite override of src/main/faq/FaqDao.ts
// - placeholder: $N → ?
// - boolean isActive: INTEGER (0/1) 컬럼 → values.push(isActive ? 1 : 0)
// - RETURNING/ILIKE/ON CONFLICT 패턴 없음 (단순)
@Dao("FaqDao")
export class FaqDao {
  ...
  async findItemsByConditions(params: GetFaqListRequest): Promise<FaqItem[]> {
    try {
      ...
      if (params.categoryCode) {
        conditions.push(`fi.category_code = ?`);
      ...
      if (params.search && params.search.trim()) {
        ...
        conditions.push(`(
          LOWER(fi.question) LIKE ? OR
          LOWER(fi.answer) LIKE ?
        )`);
