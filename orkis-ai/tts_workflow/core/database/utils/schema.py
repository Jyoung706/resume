...
class ColumnDB:
    cid:int
    name:str
    type:str | None # could be None
    notnull:bool
    dflt_value:str | None
    pk:int # 0 if not pk, >0 if pk (order in composite key)
...
class DatabaseSchema:
    ...
    def from_schema_dict_with_descriptions(
        cls, schema_dict_with_info: Dict[str, Dict[str, Dict[str, Any]]] | Dict[str, Dict[str, ColumnInfo]]
    ) -> "DatabaseSchema":
        """
        ...
        for table_name, columns_info in schema_dict_with_info.items():
            for column_name, info in columns_info.items():
                if isinstance(info, ColumnInfo):
                    database_schema.tables[table_name].columns[column_name] = info
                else:
