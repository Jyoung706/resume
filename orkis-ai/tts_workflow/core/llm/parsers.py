...
class PythonListOutputParser(BaseOutputParser):
    ...
    def parse(self, output: str) -> Any:
        ...
        """
        exec_logger.debug(f"Parsing output with PythonListOutputParser: {output}")
        if "```python" in output:
        ...
        try:
            ...
            raise OutputParserException(f"Error parsing Python list output: {e}")


class IntentClassifierOutput(BaseModel):
    """Model for intent classifier with query rewrite output."""

    rewritten_question: str = Field(
        description="The rewritten question that is self-contained and clear"
    )
    requires_db_retrieval: bool = Field(
        description="True if the question requires database retrieval, False otherwise"
    )
...
class SelectTablesOutputParser(BaseOutputParser):
    ...
    def parse(self, output: str) -> Any:
        ...
        """
        exec_logger.debug(f"Parsing output with SelectTablesOutputParser: {output}")
        if "```json" in output:
        ...
        try:
            ...
            raise OutputParserException(f"Error parsing JSON output: {e}")
...
class GenerateCandidateFinetunedMarkDownParser(BaseOutputParser):
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        """
        exec_logger.debug(f"Parsing output with MarkDownOutputParser: {output}")
        try:
            if "```sql" in output:
            ...
            raise OutputParserException(f"Error parsing SQL markdown output: {e}")
...
class GenerateCandidateGeminiMarkDownParserCOT(BaseOutputParser):
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: A dictionary with the SQL query.
        """
        exec_logger.debug(f"Parsing output with RecapOutputParserCOT: {output}")
        plan = ""
        ...
        # 어차피 history 저장하는데 plain 넣을 필요 없어 보여서 뺐음.
...
class GeminiMarkDownOutputParserCOT(BaseOutputParser):
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: A dictionary with the SQL query.
        """
        exec_logger.debug(f"Parsing output with MarkDownOutputParserCoT: {output}")
        try:
            ...
            raise OutputParserException(f"Error parsing Gemini CoT output: {e}")
...
class ReviseGeminiOutputParser(BaseOutputParser):
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: A dictionary with the SQL query.
        """
        # exec_logger.debug(f"Parsing output with CheckerOutputParser: {output}")
        try:
            if "<FINAL_ANSWER>" in output and "</FINAL_ANSWER>" in output:
                output = output.split("<FINAL_ANSWER>")[1].split("</FINAL_ANSWER>")[0]
            ...
            raise OutputParserException(f"Error parsing revise output: {e}")
        ...
        for line in output.split("\n"):
            if ":" in line:
                try:
                    ...
                    raise OutputParserException(
                        f"Error parsing unit test evaluation: {e}, each line should be in the format 'unit test #n: Passed/Failed'"
                    )
...
def get_parser(parser_name: str) -> BaseOutputParser:
    ...
    parser_configs = {
        ...
        "intent_classifier_json": lambda: JsonOutputParser(
            pydantic_object=IntentClassifierOutput
        ),
        "filter_column": lambda: JsonOutputParser(pydantic_object=FilterColumnOutput),
        "select_tables": lambda: JsonOutputParser(
            pydantic_object=SelectTablesOutputParser
        ),
        "select_columns": lambda: JsonOutputParser(
            pydantic_object=ColumnSelectionOutput
        ),
        "select_schema": SelectTablesOutputParser,
        "generate_candidate": lambda: JsonOutputParser(
            pydantic_object=GenerateCandidateOutput
        ),
    ...
    if parser_name not in parser_configs:
        exec_logger.error(f"Invalid parser name: {parser_name}")
        raise ValueError(f"Invalid parser name: {parser_name}")

    exec_logger.info(f"Retrieving parser for: {parser_name}")
    parser = (
