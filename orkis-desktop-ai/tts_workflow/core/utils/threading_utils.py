...
def ordered_concurrent_function_calls(call_list: list) -> list:
    ...
    """
    if not call_list:  # Empty list guard
        return []
    ...
    with ThreadPoolExecutor(max_workers=len(call_list)) as executor:
        for idx, call in enumerate(call_list):
            ...
            kwargs = call["kwargs"]
            future = executor.submit(func, thread_id=idx, result_queue=result_queue, **kwargs)
            futures.append(future)
        
        # Wait for all futures to complete and catch any CriticalError
        for future in futures:
            try:
                future.result()  # This will raise if CriticalError occurred
            except CriticalError:
                raise  # Re-raise CriticalError immediately
