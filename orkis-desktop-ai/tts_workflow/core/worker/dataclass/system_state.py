...
class SystemState(BaseModel):
    ...
    _tentative_schema: Optional[Dict[str, List[str]]] = None
    ...
    @property
    def tentative_schema(self) -> Dict[str, List[str]]:
        """Lazy loading - ExecutorContext 설정 후 첫 접근 시 자동 로드"""
        if self._tentative_schema is None:
            self._tentative_schema = self._src_search.get_tentative_schema()
        return self._tentative_schema

    @tentative_schema.setter
    def tentative_schema(self, value: Dict[str, List[str]]) -> None:
        self._tentative_schema = value
